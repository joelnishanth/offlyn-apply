...
**What it does:** The main browser extension popup (~320px wide). Contains enable toggle, profile switcher (multi-profile), job info bar, Auto-Fill / cover letter actions, nav links (Manage Profile, **Find Jobs** → `jobs/jobs.html`, Tailor Resume, etc.), application stats, Ollama connection status, and a collapsible advanced section.
