...
export class OllamaClient {
  ...
  async chat(messages: Message[], options?: {
    ...
    try {
      ...
      const response = await ollamaFetch(`${this.baseUrl}/v1/chat/completions`, {
        method: 'POST',
        headers: {
  ...
  /**
   * Streaming chat — yields content chunks via onChunk callback.
   * Uses the OpenAI-compatible SSE streaming format.
   */
  ...
  async createEmbedding(text: string): Promise<number[]> {
    try {
      const response = await ollamaFetch(`${this.baseUrl}/api/embeddings`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
