...
function appendStreamingAIBubble(): { contentEl: HTMLElement; badgeEl: HTMLElement } {
  const messages = qs<HTMLElement>('#messages');
  const div = document.createElement('div');
  div.className = 'message';
  ...
  div.innerHTML = `
    <div class="avatar avatar-ai">✦</div>
    <div class="bubble bubble-ai">
      <div class="streaming-content" style="white-space:pre-wrap;"></div>
  ...
  return {
    contentEl: div.querySelector('.streaming-content') as HTMLElement,
    badgeEl: div.querySelector('.source-badge') as HTMLElement,
  };
}
...
async function submitQuestion(question: string): Promise<void> {
  ...
  const { contentEl, badgeEl } = appendStreamingAIBubble();
  contentEl.textContent = '';

  const port = browser.runtime.connect({ name: 'chat-query' });
  ...
  await new Promise<void>((resolve) => {
    port.onMessage.addListener((msg: any) => {
      if (msg.kind === 'chunk') {
        ...
        const source: AnswerSource = msg.source || 'llm';
        const sourceLabel =
          source === 'graph'   ? '⬡ Graph memory'   :
          source === 'profile' ? '👤 Profile'        :
          source === 'llm'     ? '✦ Ollama'          :
          '⚠ Error';
        const sourceClass = source === 'graph' ? 'graph' : source === 'llm' ? 'llm' : '';
        badgeEl.className = `source-badge ${sourceClass}`;
        badgeEl.textContent = sourceLabel;
        badgeEl.style.display = '';
        port.disconnect();
        resolve();
      } else if (msg.kind === 'error') {
        fullAnswer = msg.error || 'Something went wrong.';
        contentEl.textContent = fullAnswer;
    ...
    port.onDisconnect.addListener(() => {
      if (!fullAnswer) {
        contentEl.textContent = 'Connection lost. Is Ollama running?';
        badgeEl.className = 'source-badge';
        badgeEl.textContent = '⚠ Error';
        badgeEl.style.display = '';
      }
