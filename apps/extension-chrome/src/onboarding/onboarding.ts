...
import {
  saveUserProfile, getUserProfile, isPhoneDetails, isLocationDetails, formatPhone, formatLocation,
  getProfilesIndex, setActiveProfile, getActiveProfileId, listProfiles,
} from '../shared/profile';
...
/**
 * Hide progress bar and stop carousel
 */
...
function showErrorDetails(error: string, details?: any): void {
  ...
  if (errorKind === 'model-not-found') {
    ...
    html += `
      ...
      <pre style="background:#1e2a3a;color:#a3e635;padding:10px 12px;border-radius:6px;font-size:12px;margin:8px 0;white-space:pre-wrap;word-break:break-all;">OLLAMA_ORIGINS='chrome-extension://bjllpojjllhfghiemokcoknfmhpmfbph' ollama serve</pre>
      ...
      <pre style="background:#1e2a3a;color:#a3e635;padding:10px 12px;border-radius:6px;font-size:12px;margin:8px 0;white-space:pre-wrap;word-break:break-all;">echo 'export OLLAMA_ORIGINS="chrome-extension://bjllpojjllhfghiemokcoknfmhpmfbph"' >> ~/.zshrc &amp;&amp; source ~/.zshrc</pre>
    ...
    html += `
      ...
      <ul>
        ...
        <li>If you see "403 / Forbidden", restart Ollama with:
          <pre style="background:#1e2a3a;color:#a3e635;padding:8px 10px;border-radius:5px;font-size:12px;margin:6px 0;">OLLAMA_ORIGINS='chrome-extension://bjllpojjllhfghiemokcoknfmhpmfbph' ollama serve</pre>
        </li>
...
function showOllamaSetupState(state: OllamaSetupState): void {
  ...
  const installGuide = document.getElementById('ollamaInstallGuide');
  ...
  if (state === 'welcome') {
    ...
    if (continueBtn) continueBtn.disabled = true;
    if (triviaContainer) {
      triviaContainer.style.display = '';
      mountTriviaGame(triviaContainer, () => {
        destroyTriviaGame();
        mountGamePicker(triviaContainer);
        triviaContainer.scrollIntoView({ behavior: 'smooth', block: 'center' });
      });
      setTimeout(() => triviaContainer.scrollIntoView({ behavior: 'smooth', block: 'center' }), 300);
    }
  } else if (state === 'ready') {
    if (downloadArea) downloadArea.style.display = 'none';
    if (installGuide) installGuide.style.display = 'none';
...
/**
 * Validate that all AI endpoints work before marking onboarding complete.
 * Tests: generate (POST), embeddings (POST), streaming.
 * Catches CORS/Origin issues, missing models, and broken connections early.
 */
async function runAIValidation(): Promise<void> {
  setProgressReal(90, 'Validating AI connection...');

  const config = await getOllamaConfig();
  const chatModel = config.chatModel || 'llama3.2';
  const embeddingModel = config.embeddingModel || 'nomic-embed-text';

  // Test 1: POST /api/generate (non-streaming)
  setProgressReal(92, 'Testing text generation...');
  const genResp = await browser.runtime.sendMessage({
    kind: 'OLLAMA_PROXY',
    method: 'POST',
    path: '/api/generate',
    body: { model: chatModel, prompt: 'Say "ok"', stream: false, options: { num_predict: 5 } },
  }) as { ok?: boolean; status?: number; error?: string };

  if (!genResp?.ok) {
    const hint = genResp?.status === 403
      ? 'CORS blocked — please run the Offlyn AI Setup installer again.'
      : `Generate test failed (${genResp?.error || genResp?.status || 'unknown'})`;
    throw new Error(hint);
  }

  // Test 2: POST /api/embeddings
  setProgressReal(95, 'Testing embeddings...');
  const embResp = await browser.runtime.sendMessage({
    kind: 'OLLAMA_PROXY',
    method: 'POST',
    path: '/api/embeddings',
    body: { model: embeddingModel, prompt: 'test' },
  }) as { ok?: boolean; data?: any; error?: string };

  if (!embResp?.ok) {
    throw new Error(`Embedding model ${embeddingModel} not responding — is it pulled?`);
  }

  // Test 3: Streaming test via a trivial generate
  setProgressReal(97, 'Testing streaming...');
  const streamResp = await browser.runtime.sendMessage({
    kind: 'OLLAMA_PROXY',
    method: 'POST',
    path: '/api/generate',
    body: { model: chatModel, prompt: 'Hi', stream: false, options: { num_predict: 1 } },
  }) as { ok?: boolean; error?: string };

  if (!streamResp?.ok) {
    throw new Error('Streaming test failed — Ollama may need a restart.');
  }
  ...
  ollamaPollingTimer = setInterval(async () => {
    ...
    try {
      ...
      if (resp?.connected) {
        ...
        // Hide the install guide if it was showing, switch to progress UI
        const guide = document.getElementById('ollamaInstallGuide');
        if (guide) guide.style.display = 'none';
        showOllamaSetupState('installing');
        setProgressReal(15, 'Ollama detected! Checking models...');

        try {
          const config = await getOllamaConfig();
          const neededModels = [config.chatModel || 'llama3.2', config.embeddingModel || 'nomic-embed-text'];
          const modelsResp = await browser.runtime.sendMessage({ kind: 'CHECK_OLLAMA_MODELS' }) as { models?: string[] };
          const available = modelsResp?.models || [];
          const missing = neededModels.filter(m => !available.some((a: string) => a === m || a.startsWith(m + ':')));

          if (missing.length > 0) {
            setProgressReal(15, `Pulling ${missing.join(', ')}...`);
            await pullModelsViaPort();
          } else {
            console.log('[Ollama Setup] All models already present');
          }
          ...
          showOllamaSetupState('ready');
          setTimeout(() => showStep('step-upload'), 1500);
        } catch (pullErr) {
...
function getOllamaDownloadUrl(): string {
  const os = detectOS();
  if (os === 'mac') return 'https://github.com/joelnishanth/offlyn-apply/releases/latest/download/Offlyn-AI-Setup.dmg';
...
function setupOllamaStepListeners(): void {
  ...
  const os = detectOS();
  if (osLabel) {
    osLabel.textContent = os === 'mac' ? 'for macOS' : os === 'windows' ? 'for Windows' : 'for Linux';
  }
  ...
  // Download Ollama button — trigger download and show guided steps
  document.getElementById('btnOllamaDownload')?.addEventListener('click', () => {
    ...
    // Show guided install instructions instead of jumping straight to progress
    const downloadArea = document.getElementById('ollamaDownloadArea');
    const guide = document.getElementById('ollamaInstallGuide');
    if (downloadArea) downloadArea.style.display = 'none';
...
async function init(): Promise<void> {
  ...
  // Re-check Ollama whenever the tab regains focus (e.g. after the installer finishes).
  // Immediately show the progress UI and check — if connected, auto-pull and advance.
  document.addEventListener('visibilitychange', async () => {
    if (document.visibilityState !== 'visible') return;
    const ollamaStep = document.getElementById('step-ollama');
    if (!ollamaStep || ollamaStep.style.display === 'none') return;
    try {
      const resp = await browser.runtime.sendMessage({ kind: 'CHECK_OLLAMA' }) as { connected?: boolean };
      if (resp?.connected) {
        stopPolling();
        ...
        showOllamaSetupState('ready');
        setTimeout(() => showStep('step-upload'), 1500);
      } else if (!ollamaPollingTimer) {
        startOllamaPolling();
      }
    } catch {
      if (!ollamaPollingTimer) startOllamaPolling();
    }
  });
  ...
  if (parseBtn) {
    parseBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (parseGameContainer) {
          parseGameContainer.style.display = '';
          mountGamePicker(parseGameContainer);
          setTimeout(() => parseGameContainer.scrollIntoView({ behavior: 'smooth', block: 'center' }), 300);
        }
  ...
  // Done button - go to interactive tutorial
  if (doneBtn) {
