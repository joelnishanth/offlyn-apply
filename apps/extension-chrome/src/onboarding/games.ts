...
export function mountTriviaGame(container: HTMLElement, onComplete?: () => void): void {
  destroyTriviaGame();
  ...
  function renderQuestion() {
    ...
    if (idx >= questions.length) {
      ...
      if (onComplete) {
        const moreGames = el('button', { className: 'game-btn-link', style: 'display:block;margin-top:8px;' }, 'More games \u2192');
        moreGames.onclick = () => onComplete();
        body.appendChild(moreGames);
      }
      return;
    }
...
export function mount2048(container: HTMLElement): void {
  ...
  const hint = el('p', { className: 'game-2048-hint', style: 'font-size:11px;color:var(--gray-400);margin:4px 0 8px;text-align:center;' }, 'Use arrow keys or swipe to merge tiles');
...
export function mountDinoRunner(container: HTMLElement): void {
  ...
  try {
    ...
    const getURL = (typeof chrome !== 'undefined' && chrome.runtime?.getURL)
      ...
      : (typeof browser !== 'undefined' && (browser as any).runtime?.getURL)
        ? (browser as any).runtime.getURL
        : null;
    if (getURL) {
      logoImg.src = getURL('icons/monogram-nosquare.png');
      logoImg.onload = () => { logoLoaded = true; };
    }
  } catch { /* logo will fall back to geometric shape */ }
  ...
  function draw() {
    ...
    // Player — Offlyn monogram logo (falls back to a rectangle if image not loaded)
    const px = 50, ph = 20, pw = 16;
    const py = playerY - ph;
    if (logoImg && logoLoaded) {
      ctx.drawImage(logoImg, px - 2, py - 4, 24, 24);
    } else {
      ctx.fillStyle = GREEN;
